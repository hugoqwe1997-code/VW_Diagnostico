...
# URL do Gist privado com as licencas (configurado por VW Informatica)
$licUrl = "https://gist.githubusercontent.com/hugoqwe1997-code/a1138859c1cf544c2fbe8f5676c6cefd/raw/gistfile1.txt"
...
function Verify-License {
    ...
    # Chave mestra VW Informatica (sem limite, sem internet, sem validade)
    $masterKeys = @(
        "VW-MASTER-VWINFO-2026",
        "VW-ADMIN-XIQUE-7437"
    )
    if ($chave -in $masterKeys) {
        Write-Host "  Licenca MASTER ativa!" -ForegroundColor Green
        Write-Host ""
        Write-Host "  Titular  : VW Informatica (Admin)" -ForegroundColor White
        Write-Host "  Tipo     : Mestra - sem limite" -ForegroundColor White
        Write-Host "  Validade : Vitalicia" -ForegroundColor Green
        Write-Host "  PCs      : Ilimitado" -ForegroundColor Green
        Write-Host ""
        Start-Sleep -Seconds 2
        return $true
    }
    
    # Teste rapido de internet
    $temInternet = Test-Connection "8.8.8.8" -Count 1 -Quiet
    if (-not $temInternet) {
        Write-Host "  [ERRO] Sem conexao com a internet!" -ForegroundColor Red
        Write-Host "  A verificacao da licenca requer internet." -ForegroundColor Yellow
        pause
        exit
    }
    ...
    try {
        ...
        # Tentar 3 metodos diferentes de download
        $json = $null
        
        # Metodo 1: Invoke-RestMethod
        try {
            $json = Invoke-RestMethod -Uri $licUrl -UseBasicParsing -TimeoutSec 15 -ErrorAction Stop
        } catch {}
        
        # Metodo 2: Invoke-WebRequest
        if (-not $json) {
            try {
                $resp = Invoke-WebRequest -Uri $licUrl -UseBasicParsing -TimeoutSec 15 -ErrorAction Stop
                $json = $resp.Content
            } catch {}
        }
        ...
        if (-not $json) {
            try {
                ...
                $json = $wc.DownloadString($licUrl)
            } catch {}
        }
        
        if (-not $json) {
            throw "Nao foi possivel baixar as licencas"
        }
        
        # Se veio como objeto (irm converte automaticamente), usar direto
        ...
        Write-Host ""
        Write-Host "  [ERRO] Nao foi possivel verificar a licenca!" -ForegroundColor Red
        Write-Host ""
        Write-Host "  Detalhes: $($_.Exception.Message)" -ForegroundColor DarkGray
        Write-Host ""
        Write-Host "  Possiveis causas:" -ForegroundColor Yellow
        Write-Host "    - Sem conexao com a internet" -ForegroundColor White
        Write-Host "    - Firewall ou antivirus bloqueando" -ForegroundColor White
        Write-Host "    - Provedor bloqueando o acesso ao GitHub" -ForegroundColor White
        Write-Host ""
        Write-Host "  Tente novamente ou entre em contato:" -ForegroundColor Yellow
        Write-Host "  WhatsApp: (74) 99937-8375" -ForegroundColor White
        Write-Host ""
        pause
        exit
